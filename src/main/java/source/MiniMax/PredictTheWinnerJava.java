...
public class PredictTheWinnerJava {
...
/*    public boolean PredictTheWinner(int[] nums) {
    ...
    public int rec(int[] nums, int start, int end){
        ...
        return Math.max(nums[start] - rec(nums, start+1,end), nums[end] - rec(nums, start, end-1));
        ...
        public boolean PredictTheWinner(int[] nums) {
            ...
            //dp[i] is from index i with length 1,2,...nums.length in each round

            //initiate length =1, because len only depends on len-1
